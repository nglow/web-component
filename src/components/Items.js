...
export default class Items extends Component {
  ...
  template() {
    ...
    return `
      ...
      <footer>
        <button class="filterBtn" data-is-filtered="0">전체 보기</button>
        <button class="filterBtn" data-is-filtered="1">활성 보기</button>
        <button class="filterBtn" data-is-filtered="2">비활성 보기</button>
      </footer>
     `
  }
  ...
  setEvent() {
    ...
    this.addEvent('click', '.filterBtn', ({target}) => {
      this.setState({isFilter: Number(target.dataset.isFiltered)})
    })
  }
}
